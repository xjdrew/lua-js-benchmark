...
| 分类 | 测试用例 | 测试内容 |
|------|---------|---------|
| **compute** | mandelbrot, n-body, spectral-norm, fannkuch-redux | CPU 密集型数值计算 |
| **string** | fasta, json-parse, string-concat | 字符串操作 |
| **alloc** | binary-trees, object-churn | GC 压力与内存分配 |
| **table** | table-insert, array-access | 哈希表 / 对象属性操作 |
| **call** | ackermann, fibonacci, n-queens | 函数调用开销与递归 |
